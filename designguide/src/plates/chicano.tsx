 ...
 *
 * ■ 検分で直したこと
 *   初稿は顔も手も地の生成色で抜いていた。線画の作法としては
 *   「紙を残す」で筋が通るが、この図版は朱も金も面で塗ってある。
 *   面のある刷り物で肌だけ紙色だと、白い聖母に見える。
 *   グアダルーペが morena（褐色）であることは、この図像が
 *   メキシコ系の人々のものである理由そのものなので、ここは譲れない。
 *
 * ■ 描かないと決めたもの
 *   ギャング記号（帽子・拳銃・数字）は一切置かない。
 *   チカーノ・アートはそれらの寄せ集めではなく、
 *   聖母・薔薇・グレカ・placas の書き文字でできている。
 */
...
const TEAL = "#0f7b6c";
const CREAM = "#f2e9d8";
/* 肌。グアダルーペは「モレニータ（褐色の聖母）」である。
   ここを紙の色で抜くと、線画としては筋が通っても、
   色のある刷り物としては白い聖母に見えてしまう。
   朱(#c8102e)と墨のあいだの褐色を1色だけ立てる */
const MORENA = "#a8663c";
    ...
    <svg viewBox="0 0 600 800" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="チカーノ・アート様式の図版">
      ...
      <g clipPath={`url(#${P}-page)`}>
        ...
        <g transform={`translate(${CX} ${CY})`}>
          ...
          {/* 首 */}
          <path d="M -12,-96 L 12,-96 L 12,-62 L -12,-62 Z" fill={MORENA} stroke={INK} strokeWidth="1.4" />
          <g mask={`url(#${P}-mb)`}>
          ...
          {/* 肌を褐色に変えたら、首と手が地続きの一本の褐色になって
              胸が裸に見えた。手を下げて、あいだに内衣の襟を通す */}
          {/* 左右対称の山にすると、どうしても木の葉に見える。
              グアダルーペの合掌は少し傾き、左右の指先がずれている。
              その2つを入れて初めて「手」に読める */}
          <g transform="translate(2 4) rotate(-11) scale(0.94 0.9)">
            {/* 左右の半身は必ず x=0 で突き合わせる。隙間を空けると、
                下の内衣の朱がそこから覗いて、赤い筋が手を貫いて見える */}
            <path d="M -1,16 C -18,10 -25,-8 -23,-29 C -21,-43 -12,-55 -1,-62 Z" fill={MORENA} stroke={INK} strokeWidth="1.5" />
            <path d="M 1,16 C 18,10 25,-8 23,-27 C 21,-38 13,-48 1,-55 Z" fill={MORENA} stroke={INK} strokeWidth="1.5" />
            {/* 合わせ目は左右の輪郭線が突き合わさって既に出ている。
                その上にもう1本重ねると3本ぶんの太さになり、
                葉の主脈のように見えてしまうので引かない */}
            {/* 親指。右手だけ手前に倒す */}
            <path d="M 7,-8 C 16,-12 22,-4 19,5 C 16,12 8,12 4,7 Z" fill={MORENA} stroke={INK} strokeWidth="1.3" />
            {/* 指の割れは**丈の方向に**走る。
                初稿は外縁から内へ斜めに引いていたので、指の境ではなく
                葉の側脈に見え、合掌がまるごと木の葉になっていた */}
            <g stroke={INK} strokeWidth="1.1" fill="none" strokeLinecap="round" opacity="0.85">
              <path d="M -5,-54 C -9,-38 -13,-20 -15,-2" />
              <path d="M -10,-48 C -14,-34 -18,-18 -20,-4" />
              <path d="M -15,-40 C -18,-29 -21,-17 -22,-7" />
              <path d="M 5,-48 C 9,-34 13,-18 15,-2" />
              <path d="M 10,-42 C 14,-30 18,-16 20,-5" />
              <path d="M 15,-34 C 18,-25 21,-14 22,-7" />
              {/* 指のつけ根。横に1本渡すと、縦の束がそこで止まる */}
              <path d="M -21,-6 C -11,-1 11,-1 21,-6" strokeWidth="0.8" opacity="0.55" />
            </g>
            <g mask={`url(#${P}-ma)`}>
              <path d="M 5,14 C 20,8 26,-8 24,-28 C 22,-40 14,-51 4,-57 Z" fill={INK} />
            </g>
            {/* 袖口。金の輪 */}
          ...
          {/* 顔。外套に開いた窓として抜く */}
          <ellipse cx="0" cy="-116" rx="26" ry="33" fill={MORENA} stroke={INK} strokeWidth="1.6" />
          <g mask={`url(#${P}-ma)`}>
          ...
          <g transform="translate(0 206)">
            ...
            <circle cx="0" cy="-4" r="16" fill={MORENA} stroke={INK} strokeWidth="1.5" />
            <g mask={`url(#${P}-ma)`}>
