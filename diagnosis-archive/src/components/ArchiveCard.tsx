 ...
 */
export default function ArchiveCard({ card }: { card: Card }) {
  const { lang, tx } = useLang();
  const t = cardText(card, lang);

  return (
    <div className="w-full [container-type:inline-size]">
      <article className="da-card flex aspect-[34/47] w-full flex-col overflow-hidden px-[5cqw] pb-[3.5cqw] pt-[4cqw]">
        ...
        <div className="grid min-h-0 flex-1 place-items-center py-[1.5cqw]">
          <CardIcon card={card} className="aspect-square h-full max-h-[54cqw] max-w-[70%]" />
        </div>

        <div className="border-t da-hairline pt-[2.5cqw]">
        ...
        <footer className="mt-[2cqw] flex items-center justify-between border-t da-hairline pt-[2cqw]">
