      ...
      <div className="mx-auto flex h-full max-w-2xl flex-col px-4 pt-[12vh] sm:px-6">
        <div className="flex items-center gap-3 border-b-2 border-da-ink pb-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded px-2 py-1 font-mono text-[11px] tracking-[0.15em] text-da-muted hover:text-da-ink"
          >
            ESC
          </button>
        </div>
        ...
        <ul ref={listRef} className="mt-4 flex-1 overflow-y-auto pb-24" role="listbox">
          {results.map((card, i) => {
            const t = cardText(card, lang);
            return (
              <li key={card.id} role="option" aria-selected={i === cursor}>
                <button
                  type="button"
                  onClick={() => go(card)}
                ...
                >
                  <span className="font-display w-12 shrink-0 text-sm italic text-da-accent">№{card.num}</span>
