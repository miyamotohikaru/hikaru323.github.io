...
/* 豆知識。5種類を順番に回して出す。ここではその内訳と、実際の出方を見せる。
   世界・あそび・ひっくり返した話は「見出し ＋ 説明」の2行で書いてある。 */
const TIP_KINDS = [
  { k: "パソコンの豆知識",   n: 41, e: "⌘⇧4 のあと スペースを押すと ウィンドウだけ撮れる" },
  { k: "世界の豆知識",       n: 42, e: "ジェヴォンズのパラドックス — 効率が上がると消費はむしろ増える" },
  { k: "あそびの豆知識",     n: 40, e: "マジックサークル — 「ここから中は別のルール」という見えない線" },
  { k: "ひっくり返した話",   n: 35, e: "Times Newer Roman — 字幅を数%広げただけで論文が長く見える書体" },
    ...
    <main className="mx-auto max-w-[1000px] px-6">
      ...
      <section className="hair mt-16 pt-16 pb-8">
        ...
        <p className="mt-2 max-w-[520px] text-[14px]" style={{ color: "var(--muted)" }}>
          5種類を順番に。12〜25分に1回くらい、手が止まっているときだけ出ます。
          打っている最中には出しません。こすくまくんを つづけて2回タップすると、その場でも見られます。
          「へえ」で終わらないものを選んでいるので、そのまま人に話せます。
        </p>
