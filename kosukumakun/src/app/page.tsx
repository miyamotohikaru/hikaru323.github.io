...
const FEATURES = [
  ...
  { s: "roll",    t: "スクロールで 金平糖を転がす", b: "下へ動かすと右、上へ動かすと左。画面のはしまで行くと、90度まがって壁を登ります。" },
  { s: "edge",    t: "ウィンドウの縁に乗る",       b: "そっと置くと上端や左右の縁に。その窓を動かすとついてきます。" },
  { s: "edgepeek", t: "画面のはしから のぞく",     b: "手が止まると はしへどいて顔だけ出します。右クリックで4か所から選べて、つかむと縁に沿って動きます。" },
  { s: "meeting", t: "会議モードで 指してくれる",  b: "カーソルの行き先を矢印で指し、短い言葉を出します。画面共有のときに。" },
...
function Download({ label = "ダウンロード" }: { label?: string }) {
  return (
    ...
    >
      <span>{label}</span>
      <span className="text-[12px] opacity-60">711 KB</span>
    </a>
  );
}
